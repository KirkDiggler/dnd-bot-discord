...
type Service interface {
	...
	GetSessionByInviteCode(ctx context.Context, code string) (*session.Session, error)

	// UpdateSession updates session details
	UpdateSession(ctx context.Context, sessionID string, input *UpdateSessionInput) (*session.Session, error)
	...
	JoinSession(ctx context.Context, sessionID, userID string) (*session.SessionMember, error)

	// JoinSessionByCode adds a user to a session using invite code
	JoinSessionByCode(ctx context.Context, code, userID string) (*session.SessionMember, error)
	...
	ListRealmSessions(ctx context.Context, realmID string) ([]*session.Session, error)

	// ListActiveUserSessions lists active sessions for a user
	ListActiveUserSessions(ctx context.Context, userID string) ([]*session.Session, error)

	// ListActiveRealmSessions lists active sessions for a realm
	ListActiveRealmSessions(ctx context.Context, realmID string) ([]*session.Session, error)
...
type CreateSessionInput struct {
	...
	ChannelID   string
	CreatorID   string
	Settings    *session.SessionSettings // Optional, will use defaults if nil
}
...
// CreateSession creates a new game session
func (s *service) CreateSession(ctx context.Context, input *CreateSessionInput) (*session.Session, error) {
	if input == nil {
	...
	// Create session
	session := session.NewSession(sessionID, input.Name, input.RealmID, input.ChannelID, input.CreatorID)
	...
	// Add creator as DM
	session.AddMember(input.CreatorID, session.SessionRoleDM)
...
// GetSessionByInviteCode retrieves a session by invite code
func (s *service) GetSessionByInviteCode(ctx context.Context, code string) (*session.Session, error) {
...
// UpdateSession updates session details
func (s *service) UpdateSession(ctx context.Context, sessionID string, input *UpdateSessionInput) (*session.Session, error) {
	if strings.TrimSpace(sessionID) == "" {
		return nil, dnderr.InvalidArgument("session ID is required")
	}
	if input == nil {
...
func (s *service) InviteToSession(ctx context.Context, sessionID, inviterID, inviteeID string) error {
	...
	if !exists || inviter.Role != session.SessionRoleDM {
...
// JoinSession adds a user to a session
func (s *service) JoinSession(ctx context.Context, sessionID, userID string) (*session.SessionMember, error) {
	if strings.TrimSpace(sessionID) == "" {
		return nil, dnderr.InvalidArgument("session ID is required")
	}
	...
	member := session.AddMember(userID, session.SessionRolePlayer)
...
// JoinSessionByCode adds a user to a session using invite code
func (s *service) JoinSessionByCode(ctx context.Context, code, userID string) (*session.SessionMember, error) {
	if strings.TrimSpace(code) == "" {
		return nil, dnderr.InvalidArgument("invite code is required")
	}
	if strings.TrimSpace(userID) == "" {
...
func (s *service) LeaveSession(ctx context.Context, sessionID, userID string) error {
	...
	if member.Role == session.SessionRoleDM && session.Status != session.SessionStatusEnded && session.Status != session.SessionStatusPlanning {
...
// ListUserSessions lists all sessions for a user
func (s *service) ListUserSessions(ctx context.Context, userID string) ([]*session.Session, error) {
	if strings.TrimSpace(userID) == "" {
		return nil, dnderr.InvalidArgument("user ID is required")
	}
...
// ListRealmSessions lists all sessions for a realm
func (s *service) ListRealmSessions(ctx context.Context, realmID string) ([]*session.Session, error) {
...
// ListActiveUserSessions lists active sessions for a user
func (s *service) ListActiveUserSessions(ctx context.Context, userID string) ([]*session.Session, error) {
	if strings.TrimSpace(userID) == "" {
		return nil, dnderr.InvalidArgument("user ID is required")
	}
...
// ListActiveRealmSessions lists active sessions for a realm
func (s *service) ListActiveRealmSessions(ctx context.Context, realmID string) ([]*session.Session, error) {
	if strings.TrimSpace(realmID) == "" {
		return nil, dnderr.InvalidArgument("realm ID is required")
	}
