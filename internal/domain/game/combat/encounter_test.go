...
func TestCheckCombatEnd(t *testing.T) {
	t.Run("Players win when all monsters defeated", func(t *testing.T) {
		encounter := combat.NewEncounter("enc-1", "session-1", "channel-1", "Test Combat", "dm-1")
	...
	t.Run("Players lose when all players defeated", func(t *testing.T) {
		encounter := combat.NewEncounter("enc-2", "session-1", "channel-1", "Test Combat", "dm-1")
	...
	t.Run("Combat continues when both sides have active combatants", func(t *testing.T) {
		encounter := combat.NewEncounter("enc-3", "session-1", "channel-1", "Test Combat", "dm-1")
...
func TestEncounter_NextTurn_SkipsDeadCombatants(t *testing.T) {
	...
	enc.Combatants["c1"] = &combat.Combatant{
		ID:        "c1",
		Name:      "Fighter",
		IsActive:  true,
		CurrentHP: 10,
	...
	enc.Combatants["c2"] = &combat.Combatant{
		ID:        "c2",
		Name:      "Dead Goblin",
		IsActive:  true,
		CurrentHP: 0, // Dead
	...
	enc.Combatants["c3"] = &combat.Combatant{
		ID:        "c3",
		Name:      "Inactive Orc",
	...
	enc.Combatants["c4"] = &combat.Combatant{
		ID:        "c4",
		Name:      "Wizard",
		IsActive:  true,
		CurrentHP: 8,
...
func TestEncounter_Start_SkipsDeadCombatants(t *testing.T) {
	...
	enc := &combat.Encounter{
		ID:         "test-encounter",
		Status:     combat.EncounterStatusRolling,
		Combatants: make(map[string]*combat.Combatant),
