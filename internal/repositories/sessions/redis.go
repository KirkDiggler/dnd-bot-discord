...
func NewRedisRepository(cfg *RedisConfig) (*redisRepo, error) {
	if cfg == nil {
		return nil, fmt.Errorf("sessions.NewRedis %w", internal.NewMissingParamError("cfg"))
	}

	if cfg.Client == nil {
		return nil, fmt.Errorf("sessions.NewRedis %w", internal.NewMissingParamError("client"))
	}

	if cfg.TimeProvider == nil {
		return nil, fmt.Errorf("sessions.NewRedis %w", internal.NewMissingParamError("timeProvider"))
	}

	if cfg.UUIDGenerator == nil {
		return nil, fmt.Errorf("sessions.NewRedis %w", internal.NewMissingParamError("uuidGenerator"))
	}
...
func (r *redisRepo) Set(ctx context.Context, session *entities.Session) error {
	if session == nil {
		return fmt.Errorf("sessions.Set %w", internal.NewMissingParamError("session"))
	}

	if session.ID == "" {
		return fmt.Errorf("sessions.Set %w", internal.NewMissingParamError("session.ID"))
	}

	if session.UserID == "" {
		return fmt.Errorf("sessions.Set %w", internal.NewMissingParamError("session.UserID"))
	}
...
func (r *redisRepo) Create(ctx context.Context, session *entities.Session) error {
	...
	if session.ID != "" {
		return fmt.Errorf("sessions.Create %w", internal.NewInvalidParamError("ID cannot be set"))
	}

	if session.UserID == "" {
		return fmt.Errorf("sessions.Create %w", internal.NewMissingParamError("UserID"))
	}
...
func (r *redisRepo) Get(ctx context.Context, id string) (*entities.Session, error) {
	if id == "" {
		return nil, fmt.Errorf("sessions.Get %w", internal.NewMissingParamError("id"))
	}

	session, err := r.get(ctx, id)
	if err != nil {
		if errors.Is(err, internal.ErrNotFound) {
			return nil, fmt.Errorf("sessions.Get %w", NewSessionNotFoundError(id))
		}
		return nil, err
	}
...
func (r *redisRepo) Update(ctx context.Context, session *entities.Session) (*entities.Session, error) {
	if session == nil {
		return nil, fmt.Errorf("sessions.Update %w", internal.NewMissingParamError("session"))
	}

	if session.ID == "" {
		return nil, fmt.Errorf("sessions.Update %w", internal.NewMissingParamError("ID"))
	}

	if session.UserID == "" {
		return nil, fmt.Errorf("sessions.Update %w", internal.NewMissingParamError("UserID"))
	}
	...
	if err != nil {
		if errors.Is(err, internal.ErrNotFound) {
			return nil, fmt.Errorf("sessions.Get %w", NewSessionNotFoundError(session.ID))
		}
...
func (r *redisRepo) Delete(ctx context.Context, id string) error {
	if id == "" {
		return fmt.Errorf("sessions.Delete %w", internal.NewMissingParamError("id"))
	}
...
func (r *redisRepo) ListByUser(ctx context.Context, userID string) ([]*entities.Session, error) {
	if userID == "" {
		return nil, fmt.Errorf("sessions.ListByUser %w", internal.NewMissingParamError("userID"))
	}
